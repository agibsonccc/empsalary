 ...
 * (Currently not used, was debating here whether moving and renaming a file could be relevant, in this use case it's not)
...
FileUploadHandler.prototype.moveFile = function(from, to) {
	var destExists = fs.exists(to, function(destExists) {
		if (!destExists) {
			//ensure parent directory exists
			var dir = path.dirname(to), exists = fs.exists(dir, function(exists) {
				if (!exists) {
	...
	var isDir = false;
	fs.fstat(to, function(err, stats) {
		if(err)
		    isDir=false;
